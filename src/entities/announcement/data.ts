...
export async function publishAnnouncement(input: PublishAnnouncementInput) {
  const announcement = await step("1. 공지 생성", () =>
    festivalApi<{ id: string }>(`/announcements`, json("POST", { title: input.title })));
  ...
  const contentItem = await step("2. 콘텐츠 아이템 생성", () =>
    festivalApi<{ id: string }>(`/content-items`, json("POST", { contentType: "ANNOUNCEMENT", resourceType: "ANNOUNCEMENT", resourceId: announcement.id, slug })));
  const version = await step("3. 콘텐츠 버전 생성", () =>
    festivalApi<{ id: string }>(`/content-items/${contentItem.id}/versions`, json("POST", { language: "ko", body: { title: input.title } })));
  await step("4. 검수 제출", () =>
    festivalApi(`/content-versions/${version.id}/submit`, { method: "POST" }));
  await step("5. 검수 승인", () =>
    festivalApi(`/content-versions/${version.id}/reviews`, json("POST", { decision: "APPROVED", comment: "현장 공지 자동 승인" })));

  return step("6. 공지 발행", () =>
    festivalApi(`/announcements/${announcement.id}/publish`, json("POST", { contentVersionId: version.id, severity: input.severity, audience: input.audience, targetAreaIds: input.targetAreaIds, startsAt: input.startsAt, endsAt: input.endsAt || null })));
}
