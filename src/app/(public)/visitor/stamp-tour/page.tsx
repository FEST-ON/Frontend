...
import { Coins, MapPin, QrCode, Stamp, PartyPopper } from "lucide-react";
import { collectStamp, fetchPoints, fetchStampSpots } from "@/entities/coupon";
...
export default function StampTourPage() {
  ...
  return (
    <div className="px-4 pt-4 pb-6">
      <VisitorEsgHeader title={t.stampTour.title} description={t.stampTour.subtitle} />
      ...
      <section className="rounded-2xl border border-esg/30 bg-esg/5 p-4">
        ...
        <div className="mt-3 flex items-center justify-between gap-3 rounded-xl bg-esg p-3 text-white">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] font-semibold text-white/75">내 ESG 포인트</p>
              <p className="text-lg font-extrabold">{(points.data?.balance ?? 0).toLocaleString()}P</p>
            </div>
          </div>
          <Link href="/visitor/coupons/points" className="rounded-full bg-white px-3 py-1.5 text-[11px] font-bold text-esg-text">
            적립내역
          </Link>
        </div>
      </section>
      ...
      <section className="mt-5">
        <div className="mb-2 flex items-center justify-between">
          <h2 className="flex items-center gap-1.5 text-sm font-bold text-foreground">
            <Stamp className="size-4 text-esg-text" /> {t.stampTour.spotsTitle}
          </h2>
          <span className="text-[11px] font-semibold text-esg-text">{collected}/{total} 완료</span>
        </div>

        <div className="space-y-2">
          {stampQuery.isError && (
            <ErrorState
              message={queryErrorMessage(stampQuery.error, t.common.loadFailed)}
          ...
          {stampSpots.map((spot) => {
            ...
            return (
              <div key={spot.id} className="flex items-center justify-between gap-3 rounded-xl border border-esg/20 bg-card p-3">
                <div className="flex min-w-0 items-center gap-3">
                  <span className={`flex size-9 shrink-0 items-center justify-center rounded-full ${done ? "bg-esg text-white" : "bg-esg/10 text-esg-text"}`}>
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="flex items-center gap-0.5 truncate text-xs text-muted-foreground">
                      <MapPin className="size-3" /> {spot.location} · +{spot.points}P
                    </p>
                  </div>
                </div>
                {done || spot.verificationType === "SELF" ? (
                  <Button
                    ...
                    className={done ? "shrink-0 text-esg-text" : "shrink-0 bg-esg text-white hover:bg-esg/90"}
                  ...
                  <Button
                    ...
                    className={isScanning ? "shrink-0 border-esg text-esg-text" : "shrink-0 bg-esg text-white hover:bg-esg/90"}
          ...
          {scanningSpot && (
            <div className="rounded-xl border border-esg/30 bg-esg/5 p-3">
              <div className="flex items-center justify-between gap-2">
                <p className="text-xs font-semibold text-foreground">현장 QR로 스탬프 적립</p>
                <Button type="button" size="sm" variant="ghost" onClick={() => setScanningSpot(null)}>닫기</Button>
              </div>
              <div className="mt-3">
                <QrScanner
                  label={t.stampTour.scanButton}
                  onScan={(value) => collectMutation.mutate({ actionId: scanningSpot, verificationKey: value.trim() })}
                />
              </div>
              <form
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  />
                  <Button type="submit" size="sm" className="bg-esg text-white hover:bg-esg/90" disabled={!stampCode.trim() || collectMutation.isPending}>
