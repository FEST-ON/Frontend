...
export default function MembersPage() {
  ...
  const create = useMutation({
    ...
    meta: { success: "계정을 만들었어요." },
    onSuccess: () => { invalidate(); reset(); setCreating(false); },
  });
  const update = useMutation({ mutationFn: updateMembership, meta: { success: "계정 정보를 변경했어요." }, onSuccess: invalidate });
  const deactivate = useMutation({ mutationFn: deactivateMembership, meta: { success: "계정을 비활성화했어요." }, onSuccess: invalidate });

  if (memberships.isError) {
    return (
      <ErrorState
        message={`계정 목록을 불러오지 못했습니다. 최고 관리자만 접근할 수 있어요. (${queryErrorMessage(memberships.error)})`}
    ...
    <div className="space-y-4">
      ...
      <Dialog open={creating} onOpenChange={setCreating}>
        ...
        >
          ...
          <div className="space-y-1">
            <Label>역할</Label>
            <Select value={form.role} onValueChange={(value) => set("role")(String(value))}>
              <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
              <SelectContent>
                {ROLES.map((role) => <SelectItem key={role} value={role}>{ADMIN_ROLE_LABEL[role]}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="sm:col-span-2 flex items-center justify-end gap-3">
            ...
            <Button type="submit" size="sm" disabled={create.isPending}>{create.isPending ? "생성 중..." : "계정 생성"}</Button>
          </div>
        </form>
        </DialogContent>
      </Dialog>
                ...
                {rows.map((member) => (
                  <TableRow key={member.id}>
                    ...
                    <TableCell>
                      ...
                      <Select value={member.role} onValueChange={(value) => setRoleChange({ member, role: String(value) })}>
                    ...
                    <TableCell className="text-right">
                      {member.status === "ACTIVE" ? (
                        <ConfirmButton
                          size="sm"
                          variant="outline"
                          disabled={deactivate.isPending && deactivate.variables === member.id}
                          title="계정을 비활성화할까요?"
                          description={`${member.name}(${member.email}) 계정이 즉시 로그인할 수 없게 됩니다. 다시 활성화할 수 있어요.`}
                          confirmLabel="비활성화"
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          disabled={update.isPending && update.variables?.membershipId === member.id}
