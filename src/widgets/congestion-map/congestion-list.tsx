...
export function CongestionList({ zones, locale }: { zones: CongestionZone[]; locale: Locale }) {
      ...
      {zones.map((zone) => (
        <div key={zone.id} className="rounded-xl border border-border bg-card p-3.5">
          ...
          <div className="mt-1.5 flex items-center justify-between text-[11px] text-muted-foreground">
            <span>{zone.waitMinutes > 0 ? t.congestion.avgWait(zone.waitMinutes) : t.congestion.noWait}</span>
            <span>
              {t.congestion.updatedPrefix}
              {zone.updatedMinutesAgo <= 0 ? t.common.timeAgo.justNow : t.common.timeAgo.minutesAgo(zone.updatedMinutesAgo)}
            </span>
          </div>
        </div>
      ))}
